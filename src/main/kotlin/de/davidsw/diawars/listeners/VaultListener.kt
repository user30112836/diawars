...
class VaultListener(private val plugin: Diawars): Listener {
    ...
    fun onBlockPlace(event: BlockPlaceEvent) {
        ...
        if (vault == null) {
            ...
            event.player.sendMessage(mm("<red>Du kannst Diamantblöcke nur in einem Vault auf Bedrock oder Diamantblöcken platzieren!</red>"))
            return
        }

        plugin.store.vaultStore.addDiamonds(vault.team, 9)
    }
    ...
    fun onBlockBreak(event: BlockBreakEvent) {
        ...
        plugin.store.vaultStore.removeDiamonds(vault.team, 9)
    }
}